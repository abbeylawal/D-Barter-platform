...
describe("Lock", function () {
  ...
  beforeEach(async function () {
    ...
    unlockTime = currentTimestamp + 60; // 1 minute into the future
  ...
  it("Should set the right unlockTime and owner", async function () {
    expect(await lock.unlockTime()).to.equal(unlockTime);
    expect(await lock.owner()).to.equal(owner.address);
  });
  ...
  it("Should not allow non-owners to withdraw", async function () {
    ...
    await expect(lock.connect(addr1).withdraw()).to.be.revertedWith(
      "You aren't the owner"
    );
  });
});
